    <script>
                ...
                productosGrid.innerHTML = items.map(p => {
                    ...
                    if (imagen.trim() !== "") {
                        if (imagen.startsWith("http://") || imagen.startsWith("https://")) {
                            ...
                            imgSrc = BASE_URL + imagen.replace(new RegExp('^/+'), "");
                        }
                        imageHtml = `<img src="${imgSrc}" alt="${escaparHtml(nombre)}" class="w-full h-full object-cover" onerror="this.outerHTML='<div class=\\'w-full h-full bg-[#F5EEE5] flex items-center justify-center text-brownSoft\\'><i class=\\'fa-solid fa-image text-2xl\\'></i></div>'">`;
                    } else {
                        imageHtml = `<div class="w-full h-full bg-[#F5EEE5] flex items-center justify-center text-brownSoft"><i class="fa-solid fa-image text-2xl"></i></div>`;
                    }
                    ...
                    return `
                        <div class="bg-white rounded-xl p-6 shadow-sm hover:shadow-md transition-all duration-200">
                            <div class="text-center">
                                ...
                                <button onclick="manejarClickProducto(${id}, '${nombreSafe}', ${precio}, '${imgSrc}')"
        ...
        async function manejarClickProducto(id, nombre, precio, imagenUrl) {
            if (!mesaActual) {
                Swal.fire({
                    icon: "error",
                    title: "Oops...",
            ...
            try {
                const resp = await fetch(`${BASE_URL}public/api/modificadoresProducto.php?producto_id=${id}`);
                const data = await resp.json();
                
                if (data.status === "OK" && data.data.length > 0) {
                    abrirModalModificadores(id, nombre, precio, imagenUrl, data.data);
                } else {
        ...
        function abrirModalModificadores(id, nombre, precio, imagenUrl, grupos) {
            productoSeleccionado = { id, nombre, precio, imagenUrl, grupos };
            
            document.getElementById('modProductoNombre').textContent = nombre;
            document.getElementById('modProductoPrecio').textContent = `₡${precio.toLocaleString()}`;
            
            let html = '';
            grupos.forEach((grupo, gIndex) => {
                const isRequerido = grupo.requerido == 1;
                const isMultiple = grupo.seleccion_multiple == 1;
                const inputType = isMultiple ? 'checkbox' : 'radio';
                const inputName = isMultiple ? `mod_${grupo.id}[]` : `mod_${grupo.id}`;
                
                html += `
                <div class="mb-4 bg-gray-50 p-4 rounded-xl border border-gray-100">
                    <div class="flex justify-between items-center mb-2">
                        <h4 class="font-bold text-brownDark">${grupo.nombre}</h4>
                        ${isRequerido ? '<span class="text-xs bg-red-100 text-red-600 px-2 py-1 rounded font-bold">Obligatorio</span>' : '<span class="text-xs bg-gray-200 text-gray-600 px-2 py-1 rounded font-bold">Opcional</span>'}
                    </div>
                    <div class="space-y-2">
                `;
                
                grupo.opciones.forEach((opt) => {
                    const extraPrecio = parseFloat(opt.precio_adicional);
                    const labelPrecio = extraPrecio > 0 ? `<span class="text-mintGreen text-sm font-bold">+₡${extraPrecio}</span>` : '';
                    const reqAttr = (isRequerido && !isMultiple) ? 'required' : '';
                    
                    html += `
                        <label class="flex items-center justify-between p-3 mb-2 bg-white rounded-xl border border-gray-200 cursor-pointer hover:border-mintGreen transition-all shadow-sm">
                            <div class="flex items-center gap-3">
                                <input type="${inputType}" name="${inputName}" value='${JSON.stringify(opt)}' class="w-5 h-5 text-mintGreen focus:ring-mintGreen mod-input" data-precio="${extraPrecio}" ${reqAttr} onclick="toggleRadioDeselect(this)" onchange="recalcularPrecioModal()">
                                <span class="text-gray-800 font-medium">${opt.nombre}</span>
                            </div>
                            ${labelPrecio}
                        </label>
                    `;
            ...
            document.getElementById('modificadoresContenedor').innerHTML = html;
            recalcularPrecioModal();
            
            const modal = new bootstrap.Modal(document.getElementById('modalModificadores'));
            modal.show();
        }

        // Permite deseleccionar un radio button si ya estaba seleccionado
        function toggleRadioDeselect(radio) {
            if (radio.type === 'radio') {
                if (radio.dataset.wasChecked === "true") {
                    // Si ya estaba chequeado y no es obligatorio, permitimos quitarlo
                    if (!radio.hasAttribute('required')) {
                        radio.checked = false;
                        radio.dataset.wasChecked = "false";
                        radio.dispatchEvent(new Event('change')); // disparar change para recalcular
                    }
                } else {
                    // Limpiar el estado de los demás radios en el mismo grupo
        ...
        function recalcularPrecioModal() {
            if (!productoSeleccionado) return;
            
            let total = parseFloat(productoSeleccionado.precio);
            document.querySelectorAll('.mod-input:checked').forEach(input => {
                total += parseFloat(input.dataset.precio || 0);
            });
            
            document.getElementById('modProductoTotal').textContent = `Total: ₡${total.toLocaleString()}`;
        }

        function confirmarModificadores() {
            const form = document.getElementById('formModificadores');
            ...
            document.querySelectorAll('.mod-input:checked').forEach(input => {
                ...
                totalExtras += parseFloat(opt.precio_adicional);
                opcionesSeleccionadas.push(opt);
            });
            
            // Cerrar el modal
            bootstrap.Modal.getInstance(document.getElementById('modalModificadores')).hide();
            
            // Agregar el producto a la orden pero modificado
            const precioFinal = productoSeleccionado.precio + totalExtras;
        ...
        function agregarProductoModificado(nombre, precio, imagenUrl, opcionesSeleccionadas) {
            const itemExistenteIndex = ordenActual.findIndex(item => item.nombre === nombre && JSON.stringify(item.opciones_json || []) === JSON.stringify(opcionesSeleccionadas));
            
            if (itemExistenteIndex !== -1) {
                ordenActual[itemExistenteIndex].cantidad++;
            } else {
                ordenActual.push({
                    nombre,
                    precio,
                    ...
                    cantidad: 1,
                    notas: [],
                    opciones_json: opcionesSeleccionadas
                });
            }
        ...
        function actualizarOrden() {
            ...
            if (ordenActual.length === 0) {
                ...
                ordenItems.innerHTML = ordenActual.map((item, index) => {
                    ...
                    const opciones = Array.isArray(item.opciones_json) ? item.opciones_json : [];
                    const opcionesHtml = opciones.length ? `
                        <div class="mt-2 text-xs text-mintGreen font-medium">
                            ${opciones.map(opt => `<div class="flex items-center gap-1"><i class="fa-solid fa-plus text-[10px]"></i> ${opt.nombre} ${opt.precio_adicional > 0 ? '(+₡'+opt.precio_adicional+')' : ''}</div>`).join('')}
                        </div>
                    ` : '';
                    ...
                    return `
                        <div class="bg-gray-50 p-3 rounded-lg">
                            <div class="flex items-center justify-between gap-3">
                                <div class="w-12 h-12 rounded-lg overflow-hidden border border-gray-200 flex-shrink-0 bg-white flex items-center justify-center text-brownSoft">
                                    ${item.imagen && item.imagen.trim() !== '' ? `
                                        <img src="${item.imagen}" class="w-full h-full object-cover" onerror="this.outerHTML='<i class=\\'fa-solid fa-image\\'></i>'">
                                    ` : `
        ...
        async function entregarOrden(numeroMesa) {
            ...
            const {
                ...
                title: ambasListas ?
                    `¿Entregar la orden completa de la Mesa ${numeroMesa}?` : `¿Entregar ${area === 'barista' ? 'las bebida(s)' : 'la comida'} de la Mesa ${numeroMesa}?`,
                icon: 'question',
                showCancelButton: true,
            ...
            try {
                ...
                if (result.status === "OK") {
                    ...
                    await Swal.fire({
                        ...
                        icon: "success",
                        html: ambasListas ?
                            `<strong>Mesa ${numeroMesa}</strong><br>Orden completa entregada` : `<strong>Mesa ${numeroMesa}</strong><br>${area === 'barista' ? 'Bebida(s)' : 'Comida'} entregada(s)`,
                        showConfirmButton: false,
                        timer: 2000
                    });
    ...
    <div class="modal fade" id="modalModificadores" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content rounded-2xl border-0 shadow-lg">
                <div class="modal-header border-b border-[#efe7db] bg-cream rounded-t-2xl px-6 py-4">
                    <h5 class="modal-title font-bold text-brownDark text-xl">Personalizar Producto</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <form id="formModificadores" onsubmit="event.preventDefault(); confirmarModificadores();">
                    <div class="modal-body p-6">
                        <div class="flex items-center gap-4 mb-4 pb-4 border-b border-[#efe7db]">
                            <div class="w-16 h-16 rounded-xl border border-gray-100 flex items-center justify-center bg-gray-50 overflow-hidden">
                                <img src="" alt="Producto" id="modProductoImg" class="w-full h-full object-cover hidden">
                                <i class="fa-solid fa-utensils text-gray-300 text-2xl" id="modProductoIcon"></i>
                            </div>
                            <div>
                                <h3 class="text-xl font-bold text-brownDark m-0" id="modProductoNombre">Producto</h3>
                                <p class="text-brownSoft font-medium m-0" id="modProductoPrecio">₡0</p>
                            </div>
                        </div>
                        
                        <div id="modificadoresContenedor">
                            <!-- Aquí se inyectan los grupos de opciones -->
                        </div>
                    </div>
                    <div class="modal-footer border-t border-[#efe7db] p-4 bg-gray-50 rounded-b-2xl flex justify-between items-center">
                        <span class="font-bold text-xl text-brownDark" id="modProductoTotal">Total: ₡0</span>
                        <div>
                            <button type="button" class="btn bg-gray-200 text-brownSoft rounded-xl px-4 py-2 font-medium mr-2" data-bs-dismiss="modal">Cancelar</button>
                            <button type="submit" class="bg-mintGreen text-white rounded-xl px-6 py-2 font-bold transition-all shadow-sm">
                                Agregar a la Orden
                            </button>
                        </div>
                    </div>
    ...
    <!-- Script de notificaciones y utilidades -->
</body>
